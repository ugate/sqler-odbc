...
class Tester {
  ...
  static async before() {
    ...
    if (priv.mgr.db[priv.dialect].setup) {
      await priv.mgr.db[priv.dialect].setup.create.tables();
    }
    priv.created = true;
  }

  static async insertRead() {
    const date = new Date().toISOString().replace('T', ' ').replace('Z', '');
    let rslt = await priv.mgr.db[priv.dialect].create.rows({
      binds: {
        id: 1, name: 'TEST1', created: date, updated: date,
        id2: 1, name2: 'TEST2', report2: Fs.readFileSync('./test/files/audit-report.png'), created2: date, updated2: date
      }
    });
    rslt = await priv.mgr.db[priv.dialect].read.rows({ binds: { name: 'test' } });
    
    if (rslt && rslt.rows && rslt.rows[0].report) Fs.writeFileSync('img.png', rslt.rows[0].report);
  }
  ...
  static async create() {
    ...
    const txId = await priv.mgr.db[priv.dialect].beginTransaction();
...
function getConf(noPool) {
  ...
  if (!conf) {
    conf = priv.conf[priv.dialect] = JSON.parse(Fs.readFileSync(Path.join('test/conf', `${priv.dialect}.json`), 'utf8'));
    if (!priv.univ) {
      priv.univ = JSON.parse(Fs.readFileSync(Path.join('test/conf', 'priv.json'), 'utf8')).univ;
    }
    conf.univ = priv.univ;
    conf.mainPath = 'test';
...
async function rows(op, opts, testOpts) {
  ...
  let txId;
  if (!autoCommit) {
    txId = await priv.mgr.db[priv.dialect].beginTransaction();
  }
  ...
  if (op === 'read-all') {
    ...
    proms = [ priv.mgr.db[priv.dialect].auxy.all.rows(opts) ];
  } else {
    ...
    for (let i = 0, xopts; i < priv.rowCount; i++) {
      ...
      proms[i] = priv.mgr.db[priv.dialect][op].table.rows(xopts);
    }
  }
...
async function insertLob(lobFile, txId) {
  ...
  const rslt = await priv.mgr.db[priv.dialect].create.table2.rows(xopts);
  if (!txId) return rslt;
...
async function readLob(txId) {
  ...
  const rslt = await priv.mgr.db[priv.dialect].read.table2.rows(xopts);
